...
SAVE_DIR="${SAVE_DIR:-$ROOT_DIR/checkpoints_time_phys}"
...
RESULT_DIR="${RESULT_DIR:-$ROOT_DIR/results_time_phys}"
...
echo "Waiting for finetuned weights: $AFTER_MODEL_PATH"
while [[ ! -f "$AFTER_MODEL_PATH" ]]; do
  sleep 30
done

echo "Found finetuned weights, generating outputs..."
